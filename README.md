...
- `lib/node.sh`：扫描和验证 233boy 节点、清理主配置中的重复入口、选择 VLESS 节点。
...
1. 检测服务器是否已有 233boy 的 `sb`。
2. 如果已有 233boy，跳过上游安装，不覆盖、不重装、不修改原脚本。
3. 如果检测到非 233boy 的 `/etc/sing-box/config.json` 或 sing-box 核心，立即停止，不自动执行 233boy，避免覆盖原节点。
4. 只有服务器没有任何 sing-box 环境时，才下载并原样执行 `233boy/sing-box` 官方 `install.sh`。
5. 安装 `zz` 到独立目录。
6. 安装完成后只显示操作提示，**不会自动启动 `zz`**。

如果 233boy 官方脚本在已经成功生成配置后返回非零状态码，安装器会再次检查 `sb` 和 `/etc/sing-box/conf`；确认安装结果存在后继续安装 `zz`，不会再误判为失败。

> **重要：** 如果服务器原来使用 sing-box 官方配置，请先备份和迁移节点，不要直接运行 233boy 安装。`zz` 默认面向 233boy 的独立节点目录结构。
...
进入 `zz` 后先同步 233boy 节点，再配置中转。这里的“同步”只扫描和验证 `/etc/sing-box/conf/*.json`，不会把入口复制进主配置。
...
- 修复“删除转发”菜单使用空格生成列表、却按制表符解析，导致界面提示已删除但实际规则仍存在的问题。
- “查看所有转发”现在只显示真实存在的路由绑定，不再把未绑定的 SS 落地误显示成转发。
- 同步节点、绑定已有落地、取消节点中转和删除转发后，都会自动检查合并配置并重启 sing-box，避免配置文件与运行状态不同步。
- 兼容 `inbound` 为数组或字符串的旧路由规则。删除转发只删除路由绑定，SS 落地配置仍保留，方便以后重新绑定。
...
```text
duplicate inbound tag: VLESS-REALITY-xxxx.json
```

0.1.5 不再复制入口，并会在同步时清理主配置中的旧副本；出站和路由规则保持不变。
...
- 同步节点、添加或绑定 SS 落地、取消节点中转、删除转发后，都会自动执行合并配置检查、重启 sing-box、确认服务状态并保存当前主配置。
